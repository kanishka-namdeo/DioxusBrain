use dioxus::prelude::*;
use crate::store::{use_store, Block, Theme};
...
pub fn Editor() -> Element {
    ...
    let save_title = move || {
        if let Some(page_id) = &store.current_page_id {
            if !title_input.trim().is_empty() {
                store.update_page_title(page_id, &title_input.trim());
            }
        }
    ...
    let create_block = move || {
        if let Some(page_id) = &store.current_page_id {
            if let Some(page) = store.pages.get(page_id) {
                // Find the last top-level block or create new one
                let new_block_id = store.create_block(None);
                store.pages.get_mut(page_id).unwrap().blocks.push(new_block_id.clone());
                store.current_block_id = Some(new_block_id);
            }
        }
    };
                        ...
                        div { class: "text-center",
                            ...
                            div { class: "flex items-center justify-center gap-4",
                                button {
                                    ...
                                    onclick: move |_| {
                                        let page_id = store.create_page("Welcome");
                                        if let Some(page) = store.pages.get_mut(&page_id) {
                                            page.icon = Some("👋".to_string());
                                            page.blocks.push(store.create_block(None));
                                        }
                                    },
                                    "Create Welcome Page"
                                },
